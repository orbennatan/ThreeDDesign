...
translate([0, 0, rail_z0]) {
    ...
    // top horizontal rail
    translate([0, 0, rail_h - rail_bar])
        cube([deck_len, rail_bar, rail_bar]);
}
...
translate([deck_len - rail_bar, 0, rail_z0]) {
    ...
    translate([0, 0, rail_h - rail_bar])
        cube([rail_bar, deck_wid, rail_bar]);
}

// Generic straight rail run of length L, oriented along +X.
// Vertical bars sit on z=0; top rail at z=rail_h-rail_bar.
module rail_run(L) {
    color("DimGray") {
        for (x = [0 : rail_pitch : L - rail_bar])
            translate([x, 0, 0]) cube([rail_bar, rail_bar, rail_h]);
        translate([0, 0, rail_h - rail_bar])
            cube([L, rail_bar, rail_bar]);
    }
}
...
// South side: full-height stair rail.
translate([0, -1000, deck_elev]) translate([1000, 0, main_h])
    rotate([0, stair_angle, 0])
        rail_run(stair_slope_len);
...
translate([p2_x, -1000, p2_z]) rotate([0, 0, -90]) translate([0, 0, main_h])
    rotate([0, stair_angle, 0]) {
        rail_run(stair_slope_len);
        translate([0, step_w - rail_bar, 0]) rail_run(stair_slope_len);
    }
